(async function () {
      ...
      <head>
        ...
        <style>
          ...
          *{margin:0;padding:0;box-sizing:border-box}
          ...
          .container{width:100%;max-width:820px;background:#16161f;border-radius:20px;overflow:hidden;box-shadow:0 25px 70px rgba(0,0,0,0.85);border:1px solid #222;}
          .main{display:flex;flex-direction:column}
          .left{padding:50px 45px;background:#1a1a24;border-bottom:1px solid #222}
          .right{padding:50px 45px}
          h1{font-size:clamp(3.8rem,9vw,5.5rem);color:#e50914;line-height:1;margin-bottom:12px}
          h2{font-size:1.8rem;margin-bottom:22px}
          p{color:#bbb;margin-bottom:28px}
          .info div{background:#21212b;padding:15px 18px;border-radius:10px;margin-bottom:12px}
          input,textarea{width:100%;padding:15px 18px;margin-bottom:18px;background:#21212b;border:1px solid #333;border-radius:10px;color:#fff;font-size:1.02rem}
          textarea{min-height:130px;resize:vertical}
          input:focus,textarea:focus{border-color:#e50914;box-shadow:0 0 0 3px rgba(229,9,20,0.2);outline:none}
          button{width:100%;background:#e50914;color:white;border:none;padding:16px;font-size:1.1rem;font-weight:700;border-radius:10px;cursor:pointer;transition:all .3s}
          button:hover:not(:disabled){background:#f22;transform:translateY(-2px)}
          button:disabled{background:#555;cursor:not-allowed}
          #status{margin-top:18px;font-size:1rem;text-align:center;min-height:1.5em}
          @media(min-width:768px){.main{flex-direction:row}.left{border-bottom:none;border-right:1px solid #222;flex:1.35}.right{flex:1}}
        </style>
      </head>
      <body>
        <div class="container">
  ...
  async function sendAccessRequest() {
    ...
    try {
      // Payload lo más simple posible
      const message = `**Nueva solicitud de acceso**\n\n**Nombre:** ${name}\n**IP:** ${currentIP}\n**Motivo:** ${reason}\n\nDesde: ${location.hostname}`;

      await fetch(CONFIG.WEBHOOK, {
        method: "POST",
        body: JSON.stringify({ content: message, username: "Solicitud de Acceso" }),
        mode: "no-cors",
        cache: "no-store"
      });

      statusEl.style.color = "#4ade80";
      statusEl.textContent = "✅ Solicitud enviada. Revisa Discord.";
